        ...
        if (search_status == `OK`) {
            for (const message of search_results) {
                if (!existsSync(`${PATH}/logs/master.csv`)) {
                    writeFileSync(`${PATH}/logs/master.csv`, `id,author_id,user\n`)
                }
...
access(`${PATH}/analytics/figures`)
	.catch(() => mkdir(`${PATH}/analytics/figures`))
...
// ex: node runners/history "file.key" "channel_id=CHANNEL_ID"
